import { buildKeyTree, flattenTree, calculateDepthSchemaFromRaw } from './core/tree';
import { buildMatcher, matchesNode, typeCategory, SearchMode } from './core/search';
import { copyToClipboard, mustGet } from './ui/dom';
import { renderExplorerView, type ExplorerLabels } from './ui/explorer-view';
import { renderCurrentLocation, renderSearchResults, renderDepthSchema } from './ui/panels';
import {
  clearHighlights,
  focusOnNode,
...
let currentRawJson: unknown = null;
...
let schemaListEl: HTMLDivElement | null = null;
...
function renderExplorer(result: ReturnType<typeof buildKeyTree>) {
  ...
  schemaListEl = view.schemaListEl;
  openTabHandler = view.openTab;

  // 깊이별 키 개수 스키마 렌더링 (원본 JSON에서 계산)
  if (currentRawJson !== null) {
    const depthSchema = calculateDepthSchemaFromRaw(currentRawJson);
    renderDepthSchema(depthSchema, schemaListEl);
  }

  renderCurrentLocation(currentLocationPath, recentListEl, handleOpenPath, handleCopyPath);
  applySearch();
}
...
function setCurrentLocation(path: string) {
  currentLocationPath = path;
  renderCurrentLocation(path, recentListEl, handleOpenPath, handleCopyPath);
}
